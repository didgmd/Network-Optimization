...
def plot_enb(enb_list, x_max, y_max):
    ...
    fig, ax = plt.subplots()
    for enb in enb_list:
        color = "purple" if enb.nodeType == "enb" else "green"
        circle = plt.Circle((enb.posX, enb.posY), enb.radius, color=color, fill=False)
        ax.add_artist(circle)
        ax.plot(enb.posX, enb.posY, marker=".", color=color)
    ...
    image_path = os.path.join(global_folder, 'enb_topology.pdf')
...
def plot_gnb(gnb_list, x_max, y_max):
    ...
    fig, ax = plt.subplots()
    for gnb in gnb_list:
        color = "purple" if gnb.nodeType == "enb" else "green"
        circle = plt.Circle((gnb.posX, gnb.posY), gnb.radius, color=color, fill=False)
        ax.add_artist(circle)
        ax.plot(gnb.posX, gnb.posY, marker=".", color=color)
    ...
    image_path = os.path.join(global_folder, 'gnb_topology.pdf')
    plt.savefig(image_path, dpi=300, format="pdf")
    plt.show()
