...
class Node:
    def __init__(self, pos_x, pos_y, node_type, radius, pci, cell_global_id):
        self.posX = pos_x
        self.posY = pos_y
        self.nodeType = node_type
        self.radius = radius
        self.pci = pci
        self.cgi = cell_global_id
        self.neighborList = []
        self.collisionList = []
        self.confusionList = []
    ...
    def __str__(self):
        return (
            "Node: posX = %d, posY = %d, nodeType = %s, radius = %d, pci = %d, cgi = %d"
            % (
                self.posX,
                self.posY,
                self.nodeType,
...
def network_topology_construct(
    ...
    while len(nodeList) < n_nodes:
        ...
        probs = [0.3, 0.7]
        nodeType = "enb" if np.random.choice([0, 1], p=probs) == 0 else "gnb"
        ...
        # 生成基站
        objNode = Node(posX, posY, nodeType, radius, pci, cell_global_id)
        debug(objNode)
...
if __name__ == "__main__":
    ...
    for node in nodeList:
        if node.nodeType == "enb":
            ...
            center_color = "mediumturquoise"
            center_size = node.radius * 0.05
            ax.scatter(node.posX, node.posY, s=center_size, color=center_color)
            circle = plt.Circle((node.posX, node.posY), node.radius, color=fill_color, fill=True, alpha=0.4)
            ax.add_artist(circle)
            ax.plot(node.posX, node.posY, marker=".", color=center_color)

        else:
            fill_color = "lightpink" 
            center_color = "hotpink" 
            center_size = node.radius * 0.0005  
            ax.scatter(node.posX, node.posY, s=center_size, color=center_color)
            circle = plt.Circle((node.posX, node.posY), node.radius, color=fill_color, fill=True, alpha=0.4)
            ax.add_artist(circle)
            ax.plot(node.posX, node.posY, marker=".", color=center_color)
    ...
    # 创建图例
    lightblue_patch = mpatches.Patch(color='mediumturquoise', label='eNB')
    lightpink_patch = mpatches.Patch(color='hotpink', label='gNB')
    ax.legend(handles=[lightblue_patch, lightpink_patch], loc='upper right')
    ...
    # 拼接得到图片的全路径
    image_path = os.path.join(current_path, 'Topology.svg')
